...
export default function TeamsRanking() {
  ...
  return (
    <div className="min-h-screen bg-[#f6f7fa] p-4" dir="rtl" style={{ fontFamily: 'Cairo, sans-serif' }}>
      {/* الهيدر */}
      <div className="flex items-center justify-between p-4 rounded-md mb-6 bg-[#ef4343]">
        <div className="text-white text-2xl font-bold">ترتيب الفرق المشاركة</div>
        <button className="bg-white text-[#ef4343] px-4 py-2 rounded shadow hover:bg-gray-100 font-medium flex items-center gap-2">
      ...
      <div className="flex items-center justify-center gap-6 mb-6">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="radio" name="age" checked={age === '16'} onChange={() => setAge('16')} className="text-[#ef4343]" />
          <span className="text-gray-700 font-medium">تحت 16 سنة</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="radio" name="age" checked={age === '18'} onChange={() => setAge('18')} className="text-[#ef4343]" />
          <span className="text-gray-700 font-medium">تحت 18 سنة</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="radio" name="age" checked={age === 'all'} onChange={() => setAge('all')} className="text-[#ef4343]" />
      ...
      <div className="bg-white rounded-xl shadow p-0 max-w-5xl mx-auto overflow-x-auto">
        <table className="w-full text-center border-collapse">
          <thead>
            <tr className="bg-[#ef4343] text-white text-base">
          ...
          <tbody>
            {teams.map((team, idx) => (
              <tr key={team.id} className="border-b hover:bg-gray-50">
                <td className="py-2 font-bold text-[#ef4343]">{idx + 1}</td>
                ...
                <td className="py-2 font-bold text-[#ef4343]">{team.points}</td>
              </tr>
            ))}
          </tbody>
        </table>
